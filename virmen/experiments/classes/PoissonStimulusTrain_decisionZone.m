classdef PoissonStimulusTrain_decisionZone < handle
 
    %%%%% pre-generate stimulus: generatePoissonStimuli('poisson_strips.mat',@PoissonStripsV6)
  ...
  methods
    ...
    %----- Constructor
    function obj = PoissonStimulusTrain_decisionZone(targetNTrials, trialDuplication, trialDispersion)
      ...
      obj.default.cfg.decisionZone   = nan;
      
      % Stimulus train data structure
      obj.default.stim.cuePos       = cell(size(PoissonStimulusTrain_decisionZone.CHOICES));
      obj.default.stim.cueCombo     = nan(numel(PoissonStimulusTrain_decisionZone.CHOICES), 0);
    ...
    function modified = configure(obj, lCue, cueVisAt, cueDensityPerM, cueMeanRatio, maxNumCues, cueMinSeparation, numPanSession,decisionZone)
      if nargin<9
        decisionZone=0;
      end
      ...
      while obj.cfgIndex <= numel(obj.config)
        if      obj.config(obj.cfgIndex).lCue         == lCue               ...
            ...
            &&  obj.config(obj.cfgIndex).decisionZone == decisionZone
          break;
        end
  ...
  methods (Access = protected)
    ...
    function stim = poissonTrains(obj, cfg, index)
      ...
      no_cues_in_decision_zone_flag=1;
      while no_cues_in_decision_zone_flag   % there should be at least one cue in the DZ
        ...
        no_cues_in_decision_zone_flag=(isempty(cuePos) || max(cuePos)<cfg.decisionZone);
      end


      for iSide = 1:numel(meanNumCues)
        addtoOtherSide{iSide} = stim.cuePos{iSide}(stim.cuePos{iSide}<cfg.decisionZone);
      end
      for iSide = 1:numel(meanNumCues)
          Otherside=-(iSide-1.5)+1.5;
          stim.cuePos{iSide}=sort([stim.cuePos{iSide} addtoOtherSide{Otherside}]);
          cueRange              = numel(cuePos) + (1:numel(stim.cuePos{iSide}));
          cuePos(cueRange)      = stim.cuePos{iSide};
          cueSide(cueRange)     = iSide;
      end
      ...
      stim.cueCombo             = false(numel(PoissonStimulusTrain_decisionZone.CHOICES), numel(cueSide));
...
%                   ;
  ...
  methods (Static)
    ...
    function obj = loadobj(frozen)
      ...
      obj               = PoissonStimulusTrain_decisionZone();
