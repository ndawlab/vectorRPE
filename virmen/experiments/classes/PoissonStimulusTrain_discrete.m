classdef PoissonStimulusTrain_discrete < handle
  ...
  methods
    ...
    %----- Constructor
    function obj = PoissonStimulusTrain_discrete(targetNTrials, trialDuplication, trialDispersion)
      ...
      obj.default.stim.cuePos       = cell(size(PoissonStimulusTrain_discrete.CHOICES));
      obj.default.stim.cueCombo     = nan(numel(PoissonStimulusTrain_discrete.CHOICES), 0);
    ...
    function modified = configure(obj, lCue, cueVisAt, cueMeanCount, cueMeanRatio, maxNumCues, cueMinSeparation, numPanSession, FracEdgeTrials, EdgeProbDef)
      ...
      while obj.cfgIndex <= numel(obj.config)
        if      obj.config(obj.cfgIndex).lCue         == lCue               ...
            ...
            &&  obj.config(obj.cfgIndex).meanCount    == cueMeanCount       ...
...
%         end
        ...
        lEffective = cfg.lCue - cfg.cueVisAt - (nCuesTotal - 1) * cfg.minCueSep;
        stim_cuePos_all = cfg.cueVisAt + sort(rand(1, nCuesTotal)) * lEffective    ...
...
% Store canonical (bit pattern) representation of cue presence
      ...
      stim.cueCombo             = false(numel(PoissonStimulusTrain_discrete.CHOICES), numel(cueSide));
...
%                   ;
  ...
  methods (Static)
    ...
    function obj = loadobj(frozen)
      ...
      obj               = PoissonStimulusTrain_discrete();
