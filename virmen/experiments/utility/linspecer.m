    ...
    switch lower(varargin{1})
        case {'qualitative','qua'}
            if N>12 % go home, you just can't get this.
                warning('linspecer:limits', 'qualitiative is not possible for greater than 12 items, please reconsider');
            else
%                 if N>9
%                     warning('linspecer:qualitative', ['Default may be nicer for ' num2str(N) ' for clearer colors use: whitebg(''black''); ']);
%                 end
            end
            qualFlag = 1;
        case {'sequential','seq'}
            ...
            return;
        otherwise
            warning('linspecer:arguments', ['parameter ''' varargin{1} ''' not recognized']);
    end
end      
