...
for k=1:length(test_trials_folds)
    ...
    if ~isempty(inds_to_drop)
        switch drop_type
            case 'norefit'
                if isempty(trial_types_to_match)
                    [curB, dev, stats] = glmfit(cur_Xtrain,cur_Ytrain,'normal','constant','on');
                else
                    ...
                    [curB, dev, stats] = glmfit(cur_Xtrain,cur_Ytrain,'normal','constant','on','weights',weights_timepoints);
                end
            ...
            case 'refit'
                ...
                if isempty(trial_types_to_match)
                    [curB, dev, stats] = glmfit(cur_Xtrain(:,inds_to_use ),cur_Ytrain,'normal','constant','on');
                else
                    ...
                    [curB, dev, stats] = glmfit(cur_Xtrain(:,inds_to_use ),cur_Ytrain,'normal','constant','on','weights',weights_timepoints);
                end
...
if nargout>2
    ...
    if isempty(trial_types_to_match)
        [B_all, dev, stats] = glmfit(cur_Xall ,cur_Yall ,'normal','constant','on');
    else
        ...
        [B_all, dev, stats] = glmfit(cur_Xall ,cur_Yall,'normal','constant','on','weights',weights_timepoints);
    end
    if nargout > 3
        covb = stats.covb; 
    end
