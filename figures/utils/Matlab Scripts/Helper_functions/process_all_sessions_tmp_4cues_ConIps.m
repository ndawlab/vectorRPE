...
function [rel_contrib_all,Fstat_all,R2_all,sesscellnum,term_names,res_cell_predicted,B_all,CovB_mat, pred_inds_cell_all] = process_all_sessions_tmp_4cues_ConIps(res_cell,period,approach,Qvals_cell,onpcaflag)
...
for sessctr = 1:numsessions
    [pred_allmat,pred_inds_cell,term_names,trials_inds_cell] = make_predictor_matrix_tmp_4cues_ConIps(res_cell(sessctr),period,lens_sides_num(sessctr));
    ...
    if ~strcmp(period,'outcome')
        if ~onpcaflag
            [relative_contrib,Fstat_mat,R2_vec,predicted_gcamp,B_vec,cur_covb_mat, pred_inds_cell_opt] = process_encoding_model_v2(pred_allmat, pred_inds_cell, neural_act_mat, pred_types_cell,approach);
        else
    ...
    else %,predicted_gcamp
        [relative_contrib,Fstat_mat,R2_vec, B_vec] = process_reward_response(pred_allmat, double(res_cell(sessctr).is_succ_gd), neural_act_mat,approach);
    end
    
    rel_contrib_all = [rel_contrib_all;relative_contrib];
    Fstat_all       = [Fstat_all;Fstat_mat];
    R2_all = [R2_all ;R2_vec];
    CovB_mat{end+1} = cur_covb_mat; 
    B_all{end+1} = B_vec;
