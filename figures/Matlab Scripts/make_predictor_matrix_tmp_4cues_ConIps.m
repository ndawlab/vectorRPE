...
%includes: 4 cue kernels: ipsi cues and contra cues for either high or low Qvalues (median split)

function [pred_allmat,pred_inds_cell,term_names,trials_inds_cell] = make_predictor_matrix_tmp_4cues_ConIps(res_cell,period,ImplantSide)
...
% Qvals_vec_split = Qvals_vec >median(Qvals_vec); % 1 for high Qvals, 0 for low Qvals
...
%         prev_Ch_isGood_cell{k}    = zeros(length(curinds),1) + double(res_cell.prev_choices_gd(k))-1; %0 is left, 1 is right
        ...
        %make Cue predictors
       % grp1 : high qals, grp 2: low Qvals
...
%         cur_contra_cues_grp1 = cur_contra_cues*(Qvals_vec_split(k)==1);
%         cur_contra_cues_grp2 = cur_contra_cues*(Qvals_vec_split(k)==0);
%         
%         cur_ipsi_cues_grp1 = cur_ipsi_cues*(Qvals_vec_split(k)==1);
%         cur_ipsi_cues_grp2 = cur_ipsi_cues*(Qvals_vec_split(k)==0);
